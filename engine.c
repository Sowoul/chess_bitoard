...
const int pawn_table[64] = {
    0,  0,  0,  0,   0,   0,  0,  0,  50, 50, 50,  10, 10, 50,  50, 50,
    10, 10, 20, 30,  30,  20, 10, 10, 5,  5,  10,  35, 35, 10,  5,  5,
    0,  0,  0,  20,  20,  0,  0,  0,  5,  -5, -10, 0,  0,  -10, -5, 5,
    5,  10, 10, -20, -20, 10, 10, 5,  0,  0,  0,   0,  0,  0,   0,  0};

const int knight_table[64] = {
    -50, -40, -30, -30, -30, -30, -40, -50, -40, -20, 0,   0,   0,
    0,   -20, -40, -30, 0,   20,  15,  15,  20,  0,   -30, -30, 5,
...
static inline bool same(const board *BOARD1, const board *BOARD2) {
...
void setupBoard(board *BOARD) {
  ...
  BOARD->WHITE_PIECES = BOARD->WHITE_ROOKS | BOARD->WHITE_KNIGHTS |
                        BOARD->WHITE_BISHOPS | BOARD->WHITE_QUEEN |
                        BOARD->WHITE_KING | BOARD->WHITE_PAWNS;
  BOARD->BLACK_PIECES = BOARD->BLACK_ROOKS | BOARD->BLACK_KNIGHTS |
                        BOARD->BLACK_BISHOPS | BOARD->BLACK_QUEEN |
                        BOARD->BLACK_KING | BOARD->BLACK_PAWNS;
}
...
void toggleMove(board *BOARD) { BOARD->turn = !BOARD->turn; }

bool getTurn(board *BOARD) { return BOARD->turn; }
...
bool is_valid_queen_move(int from, int to, uint64_t occupancy) {
  return is_valid_rook_move(from, to, occupancy) ||
         is_valid_bishop_move(from, to, occupancy);
}

bool is_valid_pawn_move(int from, int to, bool white, uint64_t occupancy,
                        uint64_t opponent) {
  int from_row = from / 8, from_col = from % 8;
  int to_row = to / 8, to_col = to % 8;
...
bool move(board *BOARD, const char *from, const char *to) {
  ...
  if (whiteTurn) {
    ...
    if (valid && (to_mask & enemy)) {
      if (to_mask & BOARD->BLACK_PAWNS)
        BOARD->BLACK_PAWNS ^= to_mask;
      if (to_mask & BOARD->BLACK_KNIGHTS)
        BOARD->BLACK_KNIGHTS ^= to_mask;
      if (to_mask & BOARD->BLACK_BISHOPS)
        BOARD->BLACK_BISHOPS ^= to_mask;
      if (to_mask & BOARD->BLACK_ROOKS)
        BOARD->BLACK_ROOKS ^= to_mask;
      if (to_mask & BOARD->BLACK_QUEEN)
        BOARD->BLACK_QUEEN ^= to_mask;
      if (to_mask & BOARD->BLACK_KING)
    ...
    if (valid && (to_mask & enemy)) {
      if (to_mask & BOARD->WHITE_PAWNS)
        BOARD->WHITE_PAWNS ^= to_mask;
      if (to_mask & BOARD->WHITE_KNIGHTS)
        BOARD->WHITE_KNIGHTS ^= to_mask;
      if (to_mask & BOARD->WHITE_BISHOPS)
        BOARD->WHITE_BISHOPS ^= to_mask;
      if (to_mask & BOARD->WHITE_ROOKS)
        BOARD->WHITE_ROOKS ^= to_mask;
      if (to_mask & BOARD->WHITE_QUEEN)
        BOARD->WHITE_QUEEN ^= to_mask;
      if (to_mask & BOARD->WHITE_KING)
        BOARD->WHITE_KING ^= to_mask;
    }
  }
...
struct MoveList *generate_moves(board *BOARD) {
  ...
  for (int from = 0; from < 64; from++) {
    ...
    bool isPawn = false, isKnight = false, isBishop = false, isRook = false,
         isQueen = false, isKing = false;
    if (whiteTurn) {
      if (BOARD->WHITE_PAWNS & from_mask)
        isPawn = true;
      else if (BOARD->WHITE_KNIGHTS & from_mask)
        isKnight = true;
      else if (BOARD->WHITE_BISHOPS & from_mask)
        isBishop = true;
      else if (BOARD->WHITE_ROOKS & from_mask)
        isRook = true;
      else if (BOARD->WHITE_QUEEN & from_mask)
        isQueen = true;
      else if (BOARD->WHITE_KING & from_mask)
        isKing = true;
    } else {
      if (BOARD->BLACK_PAWNS & from_mask)
        isPawn = true;
      else if (BOARD->BLACK_KNIGHTS & from_mask)
        isKnight = true;
      else if (BOARD->BLACK_BISHOPS & from_mask)
        isBishop = true;
      else if (BOARD->BLACK_ROOKS & from_mask)
        isRook = true;
      else if (BOARD->BLACK_QUEEN & from_mask)
        isQueen = true;
      else if (BOARD->BLACK_KING & from_mask)
        isKing = true;
    }
  ...
  *num_captures = 0;
  ...
  for (int from = 0; from < 64; from++) {
    ...
    for (int to = 0; to < 64; to++) {
      ...
      else if ((white_turn ? BOARD->WHITE_KNIGHTS : BOARD->BLACK_KNIGHTS) &
               from_mask)
        valid = is_valid_knight_move(from, to);
      else if ((white_turn ? BOARD->WHITE_BISHOPS : BOARD->BLACK_BISHOPS) &
               from_mask)
        valid = is_valid_bishop_move(from, to, occupancy);
      else if ((white_turn ? BOARD->WHITE_ROOKS : BOARD->BLACK_ROOKS) &
               from_mask)
        valid = is_valid_rook_move(from, to, occupancy);
      else if ((white_turn ? BOARD->WHITE_QUEEN : BOARD->BLACK_QUEEN) &
               from_mask)
        valid = is_valid_queen_move(from, to, occupancy);
...
_Float16 evaluateBoard(board *BOARD) { return evaluatePosition(BOARD); }
...
movepair *minimax(board *BOARD, int depth, float alpha, float beta,bool maximizingPlayer) {
    if (depth <= 0) {
    ...
    Transposition *transpose = getOrDefault(BOARD, depth);
    if (transpose) {
        saved += 1;
        movepair *result = malloc(sizeof(movepair));
        result->from = strdup(transpose->best_move->from);
        result->to = strdup(transpose->best_move->to);
        // strcpy(result->from, transpose->best_move->from);
        // strcpy(result->to, transpose->best_move->to);
        result->eval = transpose->best_move->eval;
        return result;
    }

    totalCount++;
    movepair *bestMove = malloc(sizeof(movepair));
    bestMove->from = malloc(3);
    bestMove->to = malloc(3);
    ...
    while (currentMove) {
        ...
        move(newBoard, currentMove->from, currentMove->to);
        MOVES++;
        movepair *evalMove = minimax(newBoard, depth - 1, alpha, beta, !maximizingPlayer);
        if (maximizingPlayer) {
        if (evalMove->eval > bestMove->eval) {
            strcpy(bestMove->from, currentMove->from);
            strcpy(bestMove->to, currentMove->to);
    ...
    while (currentMove) {
        struct MoveList *next = currentMove->next;
        free(currentMove->from);
        free(currentMove->to);
        free(currentMove);
        currentMove = next;
    }
    ...
    if (new_transposition) {
            new_transposition->best_move = malloc(sizeof(movepair));
            new_transposition->best_move->from = malloc(3);
            new_transposition->best_move->to = malloc(3);
            strcpy(new_transposition->best_move->from, bestMove->from);
            strcpy(new_transposition->best_move->to, bestMove->to);
            new_transposition->best_move->eval = bestMove->eval;
...
void playGame(board *BOARD, int depth, int moves) {
  for (int _ = 0; _ < moves; _++) {
    ...
    printf("%s -> %s (%f) || Positions : %lu || Saved : %lu|| Collisions : "
           "%lu\n",
           best_move->from, best_move->to, (double)best_move->eval, totalCount,
           saved, collisions);
    ...
    MOVES++;
    collisions = 0;
    if (!(BOARD->BLACK_KING && BOARD->WHITE_KING))
      return;
    free_table();
  }
}
...
movepair getBestMove(){
    board BOARD;
    init_table();
    setupRandomTable();
    setupBoard(&BOARD);
    updateMasks(&BOARD);
    movepair *bestMove = searchBestMove(&BOARD, DEPTH);
    printf("%s -> %s {%f}",bestMove->from, bestMove->to, bestMove->eval);
    return *bestMove;
}
